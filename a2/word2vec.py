...
def naiveSoftmaxLossAndGradient(
    ...
    y_hat = outsideVectors.dot(centerWordVec)
    y_hat = softmax(y_hat).reshape(-1, 1)
    y[outsideWordIdx, 0] = 1

    loss = np.log(y_hat[outsideWordIdx, 0]) * -1

    gradCenterVec = outsideVectors.T.dot(y_hat - y)

    for i in range(gradOutsideVecs.shape[0]):
        if i == outsideWordIdx:
            gradOutsideVecs[i] = centerWordVec.reshape((-1, 1)).T * (y_hat[i] - 1)
        else:
            gradOutsideVecs[i] = centerWordVec.reshape((-1, 1)).T * y_hat[i]
...
def negSamplingLossAndGradient(
    ...
    sig_val = np.zeros((len(indices), 1))

    for i in range(K + 1):
        dot_val = outsideVectors[indices[i]].dot(centerWordVec) * -1
        #print(dot_val)
        sig_val[i] = sigmoid(-dot_val) if i == 0 else sigmoid(dot_val)
        if i >= 1:
            sum = sum + np.log(sigmoid(dot_val))

    loss = -1 * np.log(sigmoid(outsideVectors[outsideWordIdx].dot(centerWordVec))) - sum

    for i in range(K):
        gradCenterVec = gradCenterVec + (1 - sig_val[i + 1, 0]) * outsideVectors[indices[i + 1]]

    gradCenterVec = gradCenterVec + -1 * (1 - sig_val[0, 0]) * outsideVectors[outsideWordIdx]


    gradOutsideVecs[indices[0]] = -1 * (1 - sig_val[0]) * centerWordVec.reshape((-1, 1)).T
    for i in range(K):
        gradOutsideVecs[indices[i + 1]] = gradOutsideVecs[indices[i + 1]] + centerWordVec.reshape((-1, 1)).T * (1 - sig_val[i + 1])
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    center_wordvec = centerWordVectors[center_idx]

    for out_word in outsideWords:
        out_idx = word2Ind[out_word]
        #out_wordvec = outsideVectors[out_idx]
        loss1, gradCenterVec1, gradOutsideVecs1 = word2vecLossAndGradient(
            centerWordVec=center_wordvec,
            outsideWordIdx=out_idx,
            outsideVectors=outsideVectors,
            dataset=dataset)
        loss = loss + loss1
        gradCenterVecs[center_idx] = gradCenterVecs[center_idx] + gradCenterVec1.reshape(-1, 1).T
        gradOutsideVectors = gradOutsideVectors + gradOutsideVecs1
